<template>
  <div class="d-flex flex-column">
    ...
    <div>
      <div
        v-if="step === 'continue'"
        class="d-flex justify-content-start container ml-5 mt-5 mb-5"
      >
        <div>
      ...
      <div class="mb-5">
        <Button
          :class="showContinue && step === 'continue' ? 'd-block' : 'd-none'"
          :buttonText="step"
          @click="setStep('continue', 'appointment')"
        ></Button>
      </div>
    </div>
    <div
      v-if="step === 'appointment'"
      class="d-flex flex-column justify-content-start container ml-5 mt-5 mb-5"
    >
      <SignedUp v-if="getSignedUp"></SignedUp>
      <Form
        v-else
        :buttonClicked="registeredClicked"
        @deActiveBtn="deActiveBtn"
        @resetButton="resetSubmit"
      ></Form>
      <Button
        :class="step === 'appointment' ? 'd-block' : 'd-none'"
        :buttonText="step"
        @click="
          formIsValidated || getSignedUp
            ? setStep('appointment', 'finalize')
            : setErrors()
        "
      ></Button>
    </div>
    ...
    >
      yes
      <Button
        :class="step === 'finalize' ? 'd-block' : 'd-none'"
        :buttonText="step"
        @click="setStep('finalize', 'info')"
      ></Button>
    </div>
    <div
      v-if="step === 'info'"
  ...
  methods: {
    ...
    async signIn() {
      const response = await fetch(
        "https://identitytoolkit.googleapis.com/v1/accounts:signInWithPassword?key=AIzaSyB5wf4Jymzdk1NWwRxAmdJc4l5_-_bP_GE",
        {
          method: "POST",
          body: JSON.stringify({
      ...
      if (!response.ok) {
        ...
        console.log(
          "the login ID is:" +
            responsePack.localId +
            "--" +
            responsePack.email +
            "--" +
            responsePack.expiresIn
        );
    ...
    async signUp() {
      const response = await fetch(
        "https://identitytoolkit.googleapis.com/v1/accounts:signUp?key=AIzaSyB5wf4Jymzdk1NWwRxAmdJc4l5_-_bP_GE",
        {
          method: "POST",
          body: JSON.stringify({
            email: this.formFields.email,
    ...
    async getReservation() {
      ...
      const response = await fetch(
        `https://calendar-9af77-default-rtdb.firebaseio.com/reservations/${userId}.json?auth=` +
          this.getToken
      );

      if (!response.ok) alert("error in fetching data");
    },
    setStep(oldValue, newValue) {
      ...
      if (this.oldStep === "appointment") this.twoStepBefore = "continue";
      else if (this.oldStep === "finalize") this.twoStepBefore = "appointment";
      else this.twoStepBefore = "";
    },
    setErrors() {
      this.setValidationClicked(true);
    },
    ...
    async setUser(userId) {
      ...
      const response = await fetch(
        `https://calendar-9af77-default-rtdb.firebaseio.com/reservations/${userId}.json`,
        {
          method: "PUT",
